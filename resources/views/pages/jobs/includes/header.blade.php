<div class="jobs-page-header">
    <div class="container jobs-header-container">
        <div class="jobs-page-header-inner">
            <div class="jobs-page-header-image">
                <img src="{{ asset('storage/' .  $company->logo ) }}" alt="{{ $company->name }}">
            </div>
            <div class="jobs-page-header-content">
                <div class="jobs-page-header-content-inner">
                    <div class="jobs-page-header-content-title">
                        <h1>{{ $job->title }}</h1>
                        <h3><a href="{{ route('companies.show', $company->id) }}">{{ $company->name }}</a></h3>
                    </div>
                    <div class="jobs-page-header-content-subtitle">
                        <h2>{{ $job->location }} &middot; {{ ucwords($job->employment_location_type) }}</h2> 
                    </div>
                    <div class="jobs-page-header-content-actions">
                        <?php if(Auth::user()){ ?>
                            <?php if(Auth::user()->id == $job->user_id){ ?>                                
                                <form action="{{ route('jobs.delete', $job->id) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <a href="{{ route('jobs.edit', $job->id) }}" class="btn btn-primary">Edit Job</a>
                                    <button type="submit" class="btn btn-danger">Delete Job</button>
                                </form>
                            <?php } else { ?>
                                <?php if(Auth::user()->role == 'job-seeker'){ ?>
                                <?php if($job->applicants->contains('user_id', Auth::user()->id)){ ?>
                                    <a href="{{ route('jobs.apply.get', $job->id) }}" class="btn btn-primary">Applied</a>
                                <?php } else { ?>
                                    <a href="{{ route('jobs.apply.get', $job->id) }}" class="btn btn-primary">Apply</a>
                                <?php } ?>
                                <?php } else { ?>
                                    <a href="" class="btn btn-primary">Want to apply? Change your role</a>
                                <?php } ?>
                            <?php } ?>
                        <?php } else {?>
                            <a href="{{ route('login') }}" class="btn btn-primary">Login to Apply</a>
                        <?php } ?>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>